...
// Лого PuzzleGuard (щит + пазл) з оригінальним градієнтом із Figma
  ...
  return (
    <header className="relative z-50 w-full bg-navy">
      <div className="mx-auto flex h-[72px] max-w-[1600px] items-center justify-between px-5 md:px-8 lg:h-[102px] lg:px-16">
        {/* Логотип: іконка + напис (менші на мобільному, повні на десктопі) */}
        ...
        {/* Десктопне меню — від 1024px (Figma: Roboto 32px) */}
        <nav className="hidden items-center gap-x-12 font-sans text-[32px] lg:flex">
          ...
          <button
            ...
            className="text-white transition-colors hover:text-mint"
          >
            Support
          </button>
        </nav>

        {/* Кнопка-бургер — лише до 1024px */}
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
      ...
      {/* Випадне мобільне меню (видно, коли open=true, і лише до 1024px) */}
      {open && (
        <nav className="absolute left-0 right-0 top-full flex flex-col gap-y-5 bg-navy px-5 py-6 font-sans text-xl shadow-lg lg:hidden">
          ...
          <button
            type="button"
            onClick={handleSupport}
            className="text-left text-white transition-colors hover:text-mint"
          >
            Support
          </button>
        </nav>
